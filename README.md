...
GostTracker: 소상공인을 위한 데이터 마케팅 도우미 - 세션 흐름 기반 사용자 관심 저하 구간 분석 도구
...
👉 **https://capstone-toridos.duckdns.org/dashboard.html**
...
```html
<button data-ghost-role="purchase-btn">결제하기</button>
```

### 장바구니 삭제 버튼

```html
<button data-ghost-role="remove-from-cart" data-product-id="상품ID">
  삭제
</button>
```
...
```html
<select data-ghost-role="option-select" data-product-id="상품ID" name="size">
  <option>S</option>
  <option>M</option>
  <option>L</option>
</select>
